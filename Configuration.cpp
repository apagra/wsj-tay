...
private:
  ...
  // allow_none adds a "None" entry at the top: the second receive source is
  // optional, so its list has to offer a way back to not having one.
  void load_audio_devices (QAudio::Mode, QComboBox *, QAudioDeviceInfo *, bool allow_none = false);
  ...
  bool restart_sound_input_2_device_;
  ...
  // optional second receive source, decoded alongside the first. Null while the
  // user has not chosen one, which is the single source default.
  QAudioDeviceInfo audio_input_2_device_;
  QAudioDeviceInfo next_audio_input_2_device_;
  AudioDevice::Channel audio_input_2_channel_;
  AudioDevice::Channel next_audio_input_2_channel_;
...
QAudioDeviceInfo const& Configuration::audio_input_2_device () const {return m_->audio_input_2_device_;}
AudioDevice::Channel Configuration::audio_input_2_channel () const {return m_->audio_input_2_channel_;}
bool Configuration::restart_audio_input_2 () const {return m_->restart_sound_input_2_device_;}
...
Configuration::impl::impl (Configuration * self, QNetworkAccessManager * network_manager
  ...
  , restart_sound_input_2_device_ {false}
  ...
  , dns_lookup_id_ {-1}
{
  ...
  //
  {
    ...
    cb = bind (&Configuration::impl::update_audio_channels, this, ui_->sound_input_2_combo_box, _1, ui_->sound_input_2_channel_combo_box, false);
    connect (ui_->sound_input_2_combo_box, static_cast<void (QComboBox::*)(int)> (&QComboBox::currentIndexChanged), cb);
  ...
  audio_input_2_device_ = next_audio_input_2_device_;
...
void Configuration::impl::find_audio_devices ()
{
  ...
  //
  // retrieve the optional second audio input device
  //
  saved_name = settings_->value ("SoundIn2Name").toString ();
  if (!saved_name.isEmpty ()
      && (next_audio_input_2_device_.deviceName () != saved_name || next_audio_input_2_device_.isNull ()))
    {
      next_audio_input_2_device_ = find_audio_device (QAudio::AudioInput, ui_->sound_input_2_combo_box, saved_name);
...
void Configuration::impl::write_settings ()
{
  ...
  if (!audio_input_2_device_.isNull ())
    {
      settings_->setValue ("SoundIn2Name", audio_input_2_device_.deviceName ());
      settings_->setValue ("AudioInput2Channel", AudioDevice::toString (audio_input_2_channel_));
    }
  else
    {
      settings_->remove ("SoundIn2Name");
    }
...
void Configuration::impl::accept ()
{
  ...
  if (next_audio_input_2_channel_ != static_cast<AudioDevice::Channel> (ui_->sound_input_2_channel_combo_box->currentIndex ()))
    {
      next_audio_input_2_channel_ = static_cast<AudioDevice::Channel> (ui_->sound_input_2_channel_combo_box->currentIndex ());
    }
  ...
  // Unlike the first source, a null second device is a valid state meaning
  // "only one source", so it must not force a restart on every visit here.
  if (audio_input_2_device_ != next_audio_input_2_device_)
    {
      audio_input_2_device_ = next_audio_input_2_device_;
      restart_sound_input_2_device_ = true;
    }
  if (audio_input_2_channel_ != next_audio_input_2_channel_)
    {
      audio_input_2_channel_ = next_audio_input_2_channel_;
      restart_sound_input_2_device_ = true;
    }
...
void Configuration::impl::load_audio_devices (QAudio::Mode mode, QComboBox * combo_box
                                              , QAudioDeviceInfo * device, bool allow_none)
{
  using std::copy;
  using std::back_inserter;
