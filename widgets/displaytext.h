...
public:
  ...
  void displayDecodedText(DecodedText const& decodedText, QString const& myCall, QString const& mode,
                          ...
                          // shown at the end of the line only; deliberately not
                          // part of the decoded message, which feeds the log and
                          // the spotting reports
                          QString const& sourceTag = QString {});
...
void log(QString s);
  ...
  // Two callsigns to watch for. Each occurrence is marked where it appears,
  // rather than colouring the whole line, so the surrounding decode stays
  // readable. Either may be empty.
  void set_watched_callsigns (QString const& first, QString const& second)
  {
    m_watched1 = first.trimmed ().toUpper ();
    m_watched2 = second.trimmed ().toUpper ();
  }
...
private:
  ...
  Configuration const * m_config {nullptr};  // uninitialised here once cost a crash
  QString m_watched1;
  QString m_watched2;
