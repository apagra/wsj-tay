...
public slots:
  ...
  void doubleClickOnCallIn2(Qt::KeyboardModifiers);
  void doubleClickOnFoxQueue(Qt::KeyboardModifiers);
  void doubleClickOnFoxInProgress(Qt::KeyboardModifiers modifiers);
  // sourceTag marks which receive source produced the lines; empty for the
  // single source case so nothing is appended
  // secondary decoders only contribute lines to the display: the decode cycle,
  // its busy flag and its counters belong to the first source alone
  void readFromStdout (QProcess * proc, QString const& sourceTag, bool secondary = false);
...
private slots:
  ...
  // This fork's own help, and the base project's manual. The rest of what
  // upstream hung on the Help menu - release notes, the DXpedition and quick
  // start PDFs, the local copy of the manual - went with the fork: each one
  // either described a program this is not, or pointed at a file this does
  // not ship.
  void on_actionWSJ_TAY_Help_triggered ();
  void on_actionWSJTX_User_Guide_triggered ();
  ...
  // MSHV does the same thing: rather than duplicate a mixer, open the one
  // Windows already has, on the tab that matters.
  void on_actionWindowsSoundRecording_triggered ();
  void on_actionWindowsSoundPlayback_triggered ();
...
private:
  ...
  // separate signal because it carries a different device, sink and channel;
  // suspend, resume and reset are shared with the first source
  Q_SIGNAL void startAudioInputStream2 (QAudioDeviceInfo const&,
      int framesPerBuffer, AudioDevice * sink,
      unsigned downSampleFactor, AudioDevice::Channel) const;
  Q_SIGNAL void stopAudioInputStream2 () const;
...
private:
  ...
  // Owned by m_wideGraphDock once docked, so not a QScopedPointer: letting both
  // the dock and a scoped pointer delete it would double free on shutdown.
  WideGraph * m_wideGraph;
  QDockWidget * m_wideGraphDock;
  ...
  // second receive source, filling its own dec_data buffer; both are always
  // constructed, but the capture is only started when one is configured
  Detector * m_detector2;
  SoundInput * m_soundInput2;

  // Both detectors must be told the same T/R period. A detector left on the
  // constructor default wraps its buffer every NTMAX seconds instead of once
  // per transmission, overruns, and then drops nearly everything.
  void setDetectorPeriods (double p);
  // The report the operator typed must survive. Auto sequencing sets it from
  // every decode of the station being worked, which silently undid a manual
  // edit; Hold Rep keeps it.
  void setAutoReport (int n);
  // Beta log: one line at a time, flushed as it goes, beside the settings so
  // File > Open log directory reaches it.
  void betaLog (QString const& line);
  // Starting the second decoder is done from two places now: once at startup,
  ...
  void highlightInBothPanes (QString const& text, QColor const& bg);
  ...
  NonInheritingProcess proc_jt9_2;  // decoder for the second receive source
  ...
  bool m_in2SilenceLogged {false};
  QString m_jt9_2_key;
  QStringList m_jt9_2_args;
  ...
  QSharedMemory *mem_jt9_2 {nullptr};     // created here, unlike the first which main () owns
  QString m_QSOText;
