                                     ...
                                     QString const& sourceTag)
{
  ...
  // Mark the watched callsigns where they appear rather than colouring the
  // whole line, so the rest of the decode stays readable. Done after the line
  // is in the document, because this works on inserted text, and limited to the
  // current period so it does not rescan the whole scrollback each time.
  if (!m_watched1.isEmpty ())
    highlight_callsign (m_watched1, QColor {220, 0, 0}, QColor {255, 255, 255}, true);
  if (!m_watched2.isEmpty ())
    highlight_callsign (m_watched2, QColor {0, 70, 200}, QColor {255, 255, 255}, true);
}
