  ...
  m_user_defined {tr ("User Defined")}
{
  ...
  // That "X" sits beside Controls at the top of the waterfall and reads as a
  // close button - which is exactly what we do not want on a dock we made
  // un-closeable. It only wiped the waterfall. Hidden, not removed: the slot
  // is still there if it ever comes back under a name that says so.
