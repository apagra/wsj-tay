...
dec_data_t dec_data2;               // samples from the second receive source
int outBufSize;
int rc;
qint32  g_iptt {0};
wchar_t buffer[256];
...
// One buffer per receive source. The two receivers hear many of the same
// stations, and a line dropped from the In 2 pane because the first source had
// already printed it would destroy the whole point of having two panes.
QHash<QString, int> earlyDecodesBestSnr2;
...
MainWindow::MainWindow(QDir const& temp_directory, bool multiple,
  ...
  m_detector2 {new Detector {RX_SAMPLE_RATE, double(NTMAX), downSampleFactor, &dec_data2}},
  m_soundInput2 {new SoundInput},
  ...
  m_block_udp_status_updates {false}
{
  ...
  // Slow on purpose: this is a comfort readout, not an instrument, and the
  // first sample only records where the counters stood.
  ...
  ui->decodedTextBrowser_in2->set_configuration (&m_config, true);
  // Watch box: every pane marks lines mentioning this callsign.
  auto const watch_changed = [this] () {
      auto const a = ui->leWatchCall->text ();
      auto const b = ui->leWatchCall2->text ();
      ui->decodedTextBrowser->set_watched_callsigns (a, b);
      ui->decodedTextBrowser_in2->set_watched_callsigns (a, b);
      ui->decodedTextBrowser2->set_watched_callsigns (a, b);
    };
  connect (ui->leWatchCall, &QLineEdit::textChanged, this, watch_changed);
  connect (ui->leWatchCall2, &QLineEdit::textChanged, this, watch_changed);
  ...
  m_soundInput2->moveToThread (&m_audioThread);
  m_detector2->moveToThread (&m_audioThread);
  bool ok;
  ...
  // second receive source: suspend, resume and reset are driven by the same
  // signals as the first so the two stay in step; only start differs
  connect (this, &MainWindow::startAudioInputStream2, m_soundInput2, &SoundInput::start);
  connect (this, &MainWindow::stopAudioInputStream2, m_soundInput2, &SoundInput::stop);
  connect (this, &MainWindow::suspendAudioInputStream, m_soundInput2, &SoundInput::suspend);
  connect (this, &MainWindow::resumeAudioInputStream, m_soundInput2, &SoundInput::resume);
  connect (this, &MainWindow::reset_audio_input_stream, m_soundInput2, &SoundInput::reset);
  connect (this, &MainWindow::finished, m_soundInput2, &SoundInput::stop);
  connect (m_soundInput2, &SoundInput::error, this, &MainWindow::showSoundInError);
  connect (&m_audioThread, &QThread::finished, m_soundInput2, &QObject::deleteLater);
  connect (&m_audioThread, &QThread::finished, m_detector2, &QObject::deleteLater);
  ...
  // the second detector needs the same block size; framesWritten is
  // deliberately left unconnected, because the first source drives the decode
  // cycle and a second trigger would decode twice per period
  connect (this, &MainWindow::FFTSize, m_detector2, &Detector::setBlockSize);
  // Sync steers the offset while it is on, so turning it off has to put back
  // what it found, or the second source silently stays aligned to that one
  // station and the control looks as though it did nothing.
  connect (ui->cbSyncToSelected, &QCheckBox::toggled, this, [this] (bool on) {
      if (on) m_offsetBeforeSync = ui->sbInput2Offset->value ();
      else ui->sbInput2Offset->setValue (m_offsetBeforeSync);
    });
  connect (m_detector2, &Detector::level, this, [this] (float db, float dbmax) {
      if (!ui) return;
      ui->signal_meter_2_widget->setValue (db, dbmax);
      m_level2Db = db;
      m_level2Fresh = true;
    });

  // The waterfall is a panel of the main window rather than a window of its own.
  m_wideGraphDock = new QDockWidget {tr ("Wide Graph"), this};
  m_wideGraphDock->setObjectName ("WideGraphDock");
  m_wideGraphDock->setAllowedAreas (Qt::TopDockWidgetArea | Qt::BottomDockWidgetArea);
  // No float or close buttons on its title bar: the waterfall is the one panel
  // that must always be there, and a stray click on a small X in the corner
  // left the operator without it and no obvious way back. It can still be
  // dragged between the top and bottom of the window by its title bar.
  ...
  connect(m_wideGraph, SIGNAL(setXIT2(int)),this,SLOT(setXIT(int)));

  connect (m_fastGraph.data (), &FastGraph::fastPick, this, &MainWindow::fastPick);

  connect (this, &MainWindow::finished, m_wideGraph, &WideGraph::close);
      ...
      if (window & 1)
        {
          ...
          ui->decodedTextBrowser_in2->erase ();
        }
      if (window & 2)
        {
  ...
  connect (ui->decodedTextBrowser_in2, &DisplayText::selectCallsign, this, &MainWindow::doubleClickOnCallIn2);
  ...
  // Attribution to SQ9FVE and K1JT lives in Help > About and in AUTHORS, which
  // is where the GPL wants it; repeating it in the title bar only crowds it.
  setWindowTitle (program_title () + "  by SV1TAY - sv1tay.com");


  connect(&proc_jt9, &QProcess::readyReadStandardOutput, this, [this] () {
      readFromStdout (&proc_jt9, secondSourceActive () ? "in=1" : QString {});
    });
  connect(&proc_jt9_2, &QProcess::readyReadStandardOutput, this, [this] () {
      readFromStdout (&proc_jt9_2, "in=2", true);
    });
  // The first decoder has handlers for stderr, errors, and exit; the second had
  // only stdout. A second jt9 that died therefore took the second source with
  // it in complete silence - no message, no log line, nothing on screen - and
  // stayed dead through every restart without ever saying why. That is exactly
  // the report that came back from the first tester, unanswerable.
  connect (&proc_jt9_2, &QProcess::readyReadStandardError, this, [this] () {
      auto chunk = proc_jt9_2.readAllStandardError ();
      if (chunk.size ()) betaLog ("decoder 2 stderr: " + QString::fromLocal8Bit (chunk).trimmed ());
    });
  connect (&proc_jt9_2, &QProcess::errorOccurred, this, [this] (QProcess::ProcessError error) {
      betaLog (QString {"decoder 2 FAILED: QProcess error %1 - the second source is now silent"}
               .arg (int (error)));
    });
  connect (&proc_jt9_2, static_cast<void (QProcess::*) (int, QProcess::ExitStatus)> (&QProcess::finished),
           this, [this] (int exitCode, QProcess::ExitStatus status) {
      if (!m_valid) return;     // we are shutting down; this is expected
      betaLog (QString {"decoder 2 EXITED: code %1, %2 - the second source is now silent"}
               .arg (exitCode)
               .arg (QProcess::NormalExit == status ? "normal exit" : "crashed"));
      // Bring it back. The first decoder is still running and the operator is
      // still on the air; losing the second receiver should cost a few seconds,
      // not the rest of the session. Capped, because a decoder that dies on
      // sight would otherwise be restarted for ever.
      if (++m_decoder2Restarts <= 5)
        {
          betaLog (QString {"decoder 2 restarting (attempt %1 of 5)"}.arg (m_decoder2Restarts));
          QTimer::singleShot (2000, this, [this] { startDecoder2 (); });
        }
      else
        {
          betaLog ("decoder 2 has died 5 times; not restarting it again this session");
        }
    });
  ...
  connect(m_wideGraph, SIGNAL(setFreq3(int,int)),this,
  ...
  // Decoder for the second receive source. Always started, so enabling a second
  // input does not need a restart; it costs one idle process, because jt9 only
  // works when handed a buffer through its own shared memory segment.
  m_jt9_2_key = QApplication::applicationName () + " - in2";
  m_jt9_2_args = jt9_args;
  m_jt9_2_args[m_jt9_2_args.indexOf (QApplication::applicationName ())] = m_jt9_2_key;
  m_jt9_env = new_env;
  startDecoder2 ();
  ...
  // What a tester cannot be asked to read off the screen.
  betaLog (QString {"---- start  %1  %2"}.arg (wsj_tay_version (), revision ()));
  betaLog (QString {"settings    %1"}.arg (m_settings->fileName ()));
  betaLog (QString {"call        %1   grid %2"}.arg (m_config.my_callsign (), m_config.my_grid ()));
  betaLog (QString {"input 1     %1"}.arg (m_config.audio_input_device ().deviceName ()));
  betaLog (QString {"input 2     %1"}.arg (m_config.audio_input_2_device ().isNull ()
                                           ? QString {"(none - single source)"}
                                           : m_config.audio_input_2_device ().deviceName ()));
  betaLog (QString {"output      %1"}.arg (m_config.audio_output_device ().deviceName ()));
 ...
 // Z
  ...
  // WSJT-Z shows its About box, with the donation appeal, at every startup
  // until dismissed. It stays on the Help menu as About WSJT-Z; it does not
  // need to interrupt the start of an operating session.
...
void MainWindow::writeSettings()
{
  ...
  m_settings->setValue ("Input2Mute", ui->signal_meter_2_label->isChecked ());
  // The watch boxes are deliberately not remembered. They hold whoever is being
  // chased right now, and a callsign left over from yesterday is noise that
  // colours decodes for no reason the operator can see.
  m_settings->setValue ("HoldRpt", ui->cbHoldRpt->isChecked ());
  m_settings->setValue ("Input2Offset", ui->sbInput2Offset->value ());
  m_settings->setValue ("Input2Sync", ui->cbSyncToSelected->isChecked ());
  m_settings->setValue ("Input2SyncCall", ui->leSyncCall->text ());
...
// m_settings->setValue("ShMsgs",m_bShMsgs);
  ...
  m_settings->setValue("SplitterState3",ui->decodes_splitter->saveState());
...
void MainWindow::readSettings()
{
  ...
  m_settings->beginGroup("MainWindow");
  ui->signal_meter_2_label->setChecked (m_settings->value ("Input2Mute", false).toBool ());
  // Hold Rep starts clear at every launch, like the watch boxes. It holds a
  // report typed for one station, and a session that ended with it still
  // ticked would begin the next one sending yesterday's number.
  ui->cbHoldRpt->setChecked (false);
  // And the second source starts un-shifted. An offset is either something
  // Sync worked out for a station or something typed for one signal; carried
  // into a new session it silently pushes every other station out of the
  // decoder's window, which costs decodes without saying a word. Sync with an
  // empty callsign is the way to hold a standing delay - it measures it again
  // each time rather than trusting a number from yesterday.
  ui->sbInput2Offset->setValue (0.);
  ui->cbSyncToSelected->setChecked (m_settings->value ("Input2Sync", false).toBool ());
  ui->leSyncCall->setText (m_settings->value ("Input2SyncCall").toString ());
...
// m_bShMsgs=m_settings->value("ShMsgs",false).toBool();
  ...
  // Keyed for three panes: a layout saved when there were two leaves the third
  // with no width at all, which looks as though the pane went missing.
  auto const splitter_state = m_settings->value("SplitterState3").toByteArray();
  if (splitter_state.isEmpty() || !ui->decodes_splitter->restoreState(splitter_state)) {
    auto const third_width = qMax(1, ui->decodes_splitter->width() / 3);
    ui->decodes_splitter->setSizes({third_width, third_width, third_width});
  }
  // The two Band Activity panes start every session the same width, whatever
  // was saved. They are there to be read against each other, and two columns of
  // different widths wrap their lines differently, which makes the comparison
  // harder than it needs to be. Dragging them apart during a session still
  // works; it just does not carry over to the next one.
  {
    auto sizes = ui->decodes_splitter->sizes();
    if (sizes.size() > 1) {
      auto const both = sizes[0] + sizes[1];
      sizes[0] = both / 2;
      sizes[1] = both - sizes[0];
...
void MainWindow::setDecodedTextFont (QFont const& font)
{
  ...
  ui->decodedTextBrowser_in2->setContentFont (font);
  ...
  ui->in2_decodes_headings_label->setStyleSheet (ui->in2_decodes_headings_label->styleSheet () + style_sheet);
  if (m_msgAvgWidget) {
  ...
  return;
    ...
    case Qt::Key_F1:
      if(bAltF1F6) {
        ...
        on_actionWSJ_TAY_Help_triggered();
        return;
      }
    case Qt::Key_F2:
      if(bAltF1F6) {
...
void MainWindow::closeEvent(QCloseEvent * e)
{
  ...
  mem_jt9->detach();
  // The second decoder needs telling too. Without this it outlives the program,
  // keeps its shared memory alive, and the next start mistakes it for an
  // orphan and refuses to run.
  if (mem_jt9_2 && mem_jt9_2->isAttached ())
    {
      if (auto * dd2 = reinterpret_cast<dec_data_t *> (mem_jt9_2->data ()))
        {
          mem_jt9_2->lock ();
          dd2->ipc[1] = 999;
          mem_jt9_2->unlock ();
        }
      if (!proc_jt9_2.waitForFinished (1000)) proc_jt9_2.close ();
      mem_jt9_2->detach ();
    }
  Q_EMIT finished ();
...
// What this fork adds. F1 opens it too.
void MainWindow::on_actionWSJ_TAY_Help_triggered ()
{
  show_wsj_tay_help (this);
}

// The base project's manual, which is where the modes and every control this
// fork did not invent are explained.
//
// Upstream builds this URL from the version number and asks the server for a
// localised copy first. WSJT-Z then pointed the base of that at its groups.io
// page, where no such file has ever existed, so Online User Guide - and F1 -
// opened a 404. A plain link to the published manual cannot rot that way.
void MainWindow::on_actionWSJTX_User_Guide_triggered ()
{
  QDesktopServices::openUrl (QUrl {"https://wsjt.sourceforge.io/wsjtx-doc/wsjtx-main-2.7.0.html"});
}
...
// The Windows Sound dialog, opened on the tab that matters: Recording for the
// level of a receive card, Playback for the transmit one. mmsys.cpl takes the
// tab index as its argument. Writing a mixer of our own would mean duplicating
// something Windows already does, and getting it wrong for the many audio
// backends this station uses; MSHV takes the same approach.
void MainWindow::on_actionWindowsSoundRecording_triggered ()
{
  QProcess::startDetached ("control.exe", {"mmsys.cpl,,1"});
}

void MainWindow::on_actionWindowsSoundPlayback_triggered ()
{
  QProcess::startDetached ("control.exe", {"mmsys.cpl,,0"});
}
...
{
  if (m_hisCall == "") {
    MessageBox::warning_message(this, "WSJ-TAY",
...
{
  // Reset the per-period dupe buffers, one per source, before a file is decoded.
  if (m_mode == "FT8") {
    earlyDecodesBestSnr.clear();
    earlyDecodesBestSnr2.clear();
  }
...
void MainWindow::on_actionKeyboard_shortcuts_triggered()
{
  ...
  <tr><td><b>F1       </b></td><td>WSJ-TAY help: what this fork adds (Alt: transmit Tx6)</td></tr>
  <tr><td><b>Shift+F1  </b></td><td>Copyright Notice</td></tr>
  <tr><td><b>Ctrl+F1  </b></td><td>About WSJ-TAY</td></tr>
...
void MainWindow::on_actionSpecial_mouse_commands_triggered()
{
  ...
  <tr>
    ...
    <td><b>Click</b> to set Rx frequency.<br/>
        ...
        <b>Shift-click</b> also sets Tx frequency.<br/>
        <b>Ctrl-click</b> to set Rx and Tx frequencies together.<br/>
  ...
  if (auto * to = reinterpret_cast<char *> (mem_jt9->data()))
    {
      ...
      if(m_mode=="MSK144" or m_bFast9) {
        ...
        if (secondSourceActive () && !m_decoder2Busy
            && !ui->signal_meter_2_label->isChecked ())
          {
            // The second source shares every decode parameter with the first,
            // only the samples differ, so take the params wholesale but keep
            // this source's own sample count.
            auto const kin2 = dec_data2.params.kin;
            dec_data2.params = dec_data.params;
            dec_data2.params.kin = kin2;
            if (auto * to2 = reinterpret_cast<char *> (mem_jt9_2->data ()))
              {
                mem_jt9_2->lock ();
                memcpy (to2, (char *) dec_data2.ipc,
                        qMin (mem_jt9_2->size (), static_cast<int> (sizeof (struct dec_data))));
                auto * dd2 = reinterpret_cast<dec_data_t *> (mem_jt9_2->data ());
                // Slide this source's samples in time. An SDR arriving through
                // a browser and a virtual cable is late by its buffering, which
                // leaves its signals sitting outside the transmission window
                // the decoder searches, costing sensitivity.
                auto const shift = int (ui->sbInput2Offset->value () * 12000.);
                if (shift && kin2 > std::abs (shift))
                  {
                    auto * d2 = dd2->d2;
                    if (shift > 0)
                      {
                        memmove (d2, d2 + shift, (kin2 - shift) * sizeof (*d2));
                        memset (d2 + kin2 - shift, 0, shift * sizeof (*d2));
                      }
                    else
                      {
                        memmove (d2 - shift, d2, (kin2 + shift) * sizeof (*d2));
                        memset (d2, 0, -shift * sizeof (*d2));
                      }
                  }
                dd2->ipc[0] = m_ihsym;
                dd2->ipc[1] = 1;
                mem_jt9_2->unlock ();
                m_decoder2Busy = true;
                m_decoder2BusySince = QDateTime::currentMSecsSinceEpoch ();
              }
          }
      }
    }
...
// Mirrors what symspec.f90 reports for the first source, 10*log10 of the mean
// square and 20*log10 of the peak, so the two meters can be read against each
// other. Measures only the samples that have arrived since the last call.
void MainWindow::updateSecondSourceMeter ()
{
  // Nothing has arrived from the second source since the last check. Reading
  // its buffer here would measure whatever samples were left in it, showing a
  // level for a source that has gone silent, so report zero instead.
  if (!ui) return;
  if (m_level2Fresh)
    {
      m_level2Fresh = false;
  ...
  ui->signal_meter_2_widget->setValue (0, 0);
}

// The median, not the mean: a handful of badly decoded DTs would drag an
// average around, while the median ignores them.
void MainWindow::recordSourceDelay (float delta)
{
  if (std::abs (delta) > 2.5) return;   // beyond what a decode could mean
  m_dtDeltas.append (delta);
  while (m_dtDeltas.size () > 50) m_dtDeltas.removeFirst ();
  if (m_dtDeltas.size () < 5) return;   // too few to be worth showing
  auto sorted = m_dtDeltas;
  std::sort (sorted.begin (), sorted.end ());
  auto const median = sorted.at (sorted.size () / 2);
  ui->lblInput2Measured->setText (QString {"%1"}.arg (median, 0, 'f', 2));
  if (ui->cbSyncToSelected->isChecked () && ui->leSyncCall->text ().trimmed ().isEmpty ()
      && std::abs (median) > 0.05)
    {
      // Align to the first source. The median is what is left over after the
      // offset already in force, so correct by half of it and let successive
      // measurements settle rather than overshooting back and forth.
      ui->sbInput2Offset->setValue (ui->sbInput2Offset->value () + median / 2.);
      m_dtDeltas.clear ();      // measurements before the change no longer apply
    }
  ui->lblInput2Measured->setToolTip (tr ("Measured delay of the second source against the first, "
                                         "from %1 stations both heard. Enter it above to correct it.")
                                     .arg (m_dtDeltas.size ()));
}

// The summary belongs at the foot of the period it describes, so it waits for
...
// A second decoder that stops answering.
//
// jt9 is handed a period through shared memory and must not be given another
// until it reports <DecodeFinished>; that report is the only thing that lowers
// the flag. Lose it once - the decoder wedged, the message never read - and the
// flag stays raised for the rest of the session: the second source is never
// asked again, while its card, its meter and its waterfall marks carry on as if
// nothing were wrong. Silent, and curable only by restarting the program. It
// was found on the air, three quarters of an hour of an empty In 2 pane with a
// healthy level meter beside it.
//
// So: if the answer has not come within two periods - far longer than any
// decode takes - assume it is lost. Write the acknowledgement jt9 may be
// waiting on, lower the flag, and let the next period hand it fresh work. Both
// ways it can be stuck are covered by that, and neither costs anything if the
// decode really was just slow, because the next period will simply find it busy
// again.
//
// It is said out loud in both places that matter: a line in the In 2 pane, so
// the operator sees why it went quiet, and a line in the beta log carrying the
// state of the handshake, which is what will eventually say why it happened.
void MainWindow::checkSecondDecoder ()
{
  if (!ui || !m_decoder2Busy || !m_decoder2BusySince) return;
  if (!secondSourceActive ()) return;
  auto const period = m_TRperiod > 0. ? m_TRperiod : 15.;
  auto const waited = (QDateTime::currentMSecsSinceEpoch () - m_decoder2BusySince) / 1000.;
  if (waited < 2. * period) return;

  int ipc0 {-1}, ipc1 {-1}, ipc2 {-1}, kin {-1};
  if (mem_jt9_2 && mem_jt9_2->isAttached ())
    {
      if (auto * dd2 = reinterpret_cast<dec_data_t *> (mem_jt9_2->data ()))
        {
          ...
          ipc2 = dd2->ipc[2];
          kin = dd2->params.kin;
          dd2->ipc[2] = 1;      // the acknowledgement it may be waiting for
          mem_jt9_2->unlock ();
        }
    }
  ++m_decoder2Stalls;
  ...
  betaLog (QString {"decoder 2 STALLED: no answer for %1 s (stall %2 this session); "
                    "ipc %3/%4/%5, kin %6, process %7 - prodded, work resumes next period"}
           .arg (waited, 0, 'f', 1)
           .arg (m_decoder2Stalls)
           .arg (ipc0).arg (ipc1).arg (ipc2).arg (kin)
           .arg (QProcess::NotRunning == proc_jt9_2.state () ? "not running" : "running"));
...
// as "has a card been chosen for Input 2": in mirror mode Input 2 is empty on
// purpose and the source is Input 1's own card, read through a shifted window.
// Asking the narrower question here is what left mirror mode with audio, a
...
void MainWindow::reopenAudioDevices (bool output, bool inputs)
{
  ...
  if (inputs && secondSourceActive ())
    {
      auto const device = by_name (QAudio::AudioInput, secondSourceDevice ().deviceName ());
      if (!device.isNull ())
        {
          Q_EMIT startAudioInputStream2 (device, m_rx_audio_buffer_frames, m_detector2
...
void MainWindow::updateResourceUsage ()
{
  ...
  auto const cpu = processes_cpu_percent (ours);
  ui->lblCpuUsage->setText (cpu < 0 ? QString {"CPU --"}
                                    : QString {"CPU %1%"}.arg (cpu));

  auto const bytes = processes_ram_bytes (ours);
  ui->lblRamUsage->setText (bytes ? QString {"RAM %1 MB"}.arg (bytes / (1024 * 1024))
                                  : QString {"RAM --"});
}

// Marking the station being worked is worth nothing if it is marked in one
// column and left plain in the other: the same station then reads as two.
//
// The highlight rescans a pane's last period rather than colouring one line as
// it arrives, so this also catches lines that reached the other pane earlier -
// and the repeats at the call site catch those that arrive later, which is the
// usual case for the second source, since it decodes a moment behind.
void MainWindow::highlightInBothPanes (QString const& text, QColor const& bg)
{
  if (text.isEmpty ()) return;
  ui->decodedTextBrowser->highlight_callsign (text, bg, QColor (255, 255, 255), true);
  ui->decodedTextBrowser_in2->highlight_callsign (text, bg, QColor (255, 255, 255), true);
}

// While this is a beta, every session leaves a trace that a tester can send
// back without being asked to read anything off the screen. It is appended and
// flushed line by line, unlike the ordinary log, which is buffered and so says
// nothing at all about a session that was killed rather than closed - that
// distinction cost an afternoon once already.
//
// It sits beside the settings, so File > Open log directory reaches it.
void MainWindow::betaLog (QString const& line)
{
  QFile f {m_config.writeable_data_dir ().absoluteFilePath ("wsj-tay-beta.log")};
  if (!f.open (QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text)) return;
  QTextStream s {&f};
  // Sound card names carry the operator's own language. Written in the local
  // 8-bit codec they came back from the first tester as rows of question marks,
  // which is no use when the question is which card was selected.
  s.setCodec ("UTF-8");
  s << QDateTime::currentDateTime ().toString ("yyyy-MM-dd hh:mm:ss") << "  " << line << "\n";
  s.flush ();
}

// Start, or restart, the decoder for the second receive source.
//
// Two things here were wrong and cost a tester his second receiver. The working
// directory was never set, so this jt9 looked for ALLCALL7.TXT wherever the
// program happened to have been launched from and, when it could not find it,
// wrote one error line and stopped decoding for good - silently, and through
// every restart. And the old startup test was "attach failed AND create
// worked", so a segment still held by an orphaned jt9 made a successful attach
// skip starting the decoder altogether.
void MainWindow::startDecoder2 ()
{
  if (QProcess::NotRunning != proc_jt9_2.state ()) return;
  // Allocated here rather than in the constructor so that the first start and
  // every restart go through one path. Losing this line once left the pointer
  // uninitialised and crashed the program on startup.
  if (!mem_jt9_2) mem_jt9_2 = new QSharedMemory {m_jt9_2_key, this};

  if (!mem_jt9_2->isAttached ())
    {
      if (mem_jt9_2->attach ()) mem_jt9_2->detach ();   // not ours; take our own
      if (!mem_jt9_2->create (sizeof (dec_data2)))
        {
          LOG_ERROR ("could not obtain shared memory for the second decoder");
          betaLog ("decoder 2 NOT STARTED: shared memory unavailable - an orphaned "
                   "jt9 from an earlier session is probably still running");
          return;
        }
    }

  mem_jt9_2->lock ();
  memset (mem_jt9_2->data (), 0, sizeof (dec_data2));
  mem_jt9_2->unlock ();

  proc_jt9_2.setProcessEnvironment (m_jt9_env);
  // As for the first decoder: jt9 opens ALLCALL7.TXT by bare filename, so it
  // has to be started in the folder that holds it.
  proc_jt9_2.setWorkingDirectory (m_appDir);
  proc_jt9_2.start (QDir::toNativeSeparators (m_appDir) + QDir::separator () +
                    "jt9", m_jt9_2_args, QIODevice::ReadWrite | QIODevice::Unbuffered);
  m_decoder2Busy = false;
  m_decoder2BusySince = 0;
}
// Auto sequencing sets the report from every decode of the station being
// worked, so a report typed by hand was silently replaced a second or two
// later. Hold Rep keeps what was typed.
void MainWindow::setAutoReport (int n)
{
  if (ui->cbHoldRpt->isChecked ()) return;
  ui->rptSpinBox->setValue (n);
}
void MainWindow::setDetectorPeriods (double p)
{
  m_detector->setTRPeriod (p);
  m_detector2->setTRPeriod (p);
}

void MainWindow::readFromStdout (QProcess * proc, QString const& sourceTag, bool secondary)
{
  // Each source has its own Band Activity pane, so what one hears can be read
  // against the other. Highlighting and CQ priority stay on the first pane:
  // they drive what gets worked, which belongs to the first source.
  auto * const pane = secondary ? ui->decodedTextBrowser_in2 : ui->decodedTextBrowser;
  ...
  //
  // It used to take effect only with the multi-threaded decoder AND on top of
  // that an early decode start, a VHF band or "Reduce false decodes" - so on a
  // plain HF station with the settings as they come, the switch did nothing and
  // the same station appeared two and three times a period, a hertz apart, one
  // line carrying an AP marker and the other not. Those conditions describe
  // when upstream's threading makes duplicates likely; they were never the only
  // way to get them, since the decoder's own passes produce them too.
  //
  // Each source keeps its own record - see earlyDecodesBestSnr2 - and both are
  // cleared once per period, so a station worked every cycle still appears
  // every cycle.
  bool const hide_ft8_dupes = ui->actionHide_FT8_dupe_messages->isChecked();
  bool const dedup_on = (m_mode == "FT8") && (hide_ft8_dupes || m_diskData);
  auto & cycleBestSnr = secondary ? earlyDecodesBestSnr2 : earlyDecodesBestSnr;
  QVector<QByteArray> batch_lines;
  while (proc->canReadLine()) {
    batch_lines.append(proc->readLine());
  }
  ...
  if (dedup_on) {
    ...
    for (auto const& raw_line : batch_lines) {
      ...
      auto const cycle_it = cycleBestSnr.constFind(dupe_key);
      if (cycle_it != cycleBestSnr.constEnd()) {
        ...
        if (snr > cycle_it.value()) {
          cycleBestSnr.insert(dupe_key, snr);
        }
        continue;
      }
...
//    double fTR=float((ms%int(1000.0*m_TRperiod)))/int(1000.0*m_TRperiod);
    if(line_read.indexOf("<DecodeFinished>") >= 0) {
      if (secondary) {
        // jt9 blocks after every decode until the host acknowledges through
        // ipc[2]; decodeDone does that for the first decoder, and without the
        // same answer here the second one decodes exactly once and then waits
        // for ever. This decoder must not call decodeDone itself: that would
        // end the period while the first source is still decoding.
        if (auto * dd2 = reinterpret_cast<dec_data_t *> (mem_jt9_2->data ())) {
          mem_jt9_2->lock ();
          dd2->ipc[2] = 1;
          mem_jt9_2->unlock ();
      ...
      // ALL.TXT gets the source tag too. Without it both receivers land in one
      // file with nothing to tell them apart, and the question a tester
      // actually asks - when did the second source stop hearing anything - has
      // no answer in the only record that survives the session.
      write_all ("Rx", sourceTag.isEmpty () ? line_read.trimmed ()
                                            : line_read.trimmed () + "  " + sourceTag);
      int ntime=6;
      ...
      if (line_read.left(ntime) != m_tBlankLine && QString::fromUtf8(line_read.constData()).left(4).contains(QRegularExpression {"\\d\\d\\d\\d"})) {
          ...
          ui->decodedTextBrowser_in2->new_period ();
          ...
          if (m_config.insert_blank ()
            ...
            if (m_bandActivityRawView) {
              ...
              ui->decodedTextBrowser_in2->insertText (band.rightJustified  (40, '-'));
            } else {
              ui->decodedTextBrowser->insertLineSpacer (band.rightJustified  (40, '-'));
              ui->decodedTextBrowser_in2->insertLineSpacer (band.rightJustified  (40, '-'));
            }
            // Z
            if (m_unfilteredView && m_unfilteredView->isVisible()) {
...
//      if(m_mode=="FT8" && fTR>0.6 && fTR<0.75) decodeDone();  // Clear a hung decoder status
      ...
      // Identify the message so that one heard by both receivers is acted on
      // only once. Both panes still show it: the key governs what the program
      // does, not what it displays. The key is the message alone - the two
      // receivers report their own SNR and DT, and an SDR can sit on a slightly
      // different audio frequency, so none of those can be part of it.
      QString key;
      {
        ...
        // different moments. Without this the same message counts as two.
        key.remove (QChar {'<'}).remove (QChar {'>'});
        if (secondary && ui->cbSyncToSelected->isChecked ())
          {
            // Steer the second source onto the station being worked, so a
            // signal that sits off the nominal timing still lands inside the
            // window its decoder searches. Only that source moves; transmit
            // timing follows UTC and is left alone.
            auto const dx = ui->leSyncCall->text ().trimmed ().toUpper ();
            if (!dx.isEmpty () && key.contains (dx) && std::abs (decodedtext0.dt ()) > 0.05)
              {
                ui->sbInput2Offset->setValue (ui->sbInput2Offset->value () + decodedtext0.dt ());
              }
          }
        if (!key.isEmpty ())
          {
      ...
      ui->decodedTextBrowser->insertText(rawViewLine);
          ...
          if (!isFiltered || for_us)
          {
              ...
              // happen to contain "a1".."a9" as a substring (e.g., PA1ABC, LA2XYZ).
                // No in=1 / in=2 on screen any more: each source has its own
                // pane, so the column a line sits in already says which
                // receiver heard it. The tag dates from when both sources
                // shared one window. ALL.TXT keeps it - there they really are
                // one file.
                if (!m_bandActivityRawView) {
                  ...
                  pane->displayDecodedText(decodedtextNoAP,m_baseCall,m_mode,dxcc,
                                                             m_logBook,m_currentBand,m_config.ppfx(),
                  ...
                  if (m_pskReporterReceivers.contains(decodedtextNoAP.transmittingCall().toUpper())) {
                      pane->highlight_callsign_line(decodedtextNoAP.transmittingCall(), QColor{}, QColor{}, false, true);
                  }
              } else {
                  pane->displayDecodedText(decodedtext1,m_baseCall,m_mode,dxcc,
                                                             m_logBook,m_currentBand,m_config.ppfx(),
                                                             ui->cbCQonly->isVisible() && ui->cbCQonly->isChecked(),
                                                             haveFSpread, fSpread, bDisplayPoints, m_points, ui->cbCQonlyIncl73->isChecked(), m_config.colourAll(), distance, state, isFiltered);
                  if (m_pskReporterReceivers.contains(decodedtext1.transmittingCall().toUpper())) {
                      pane->highlight_callsign_line(decodedtext1.transmittingCall(), QColor{}, QColor{}, false, true);
                  }
              }

                  if (ui->dxCallEntry->text() == deCall && m_config.highlightDX())
                  {
                    highlightInBothPanes (ui->dxCallEntry->text(), QColor (255, 0, 0));
                  }
                }
            ...
            // Both panes have now shown the line. What follows acts on the QSO
            // in progress - auto sequencing, transmission, spotting - and must
            // run once per message however many receivers heard it. Running it
            // twice started a transmission twice and crashed inside the
            // transmit audio generator. Whichever source reports first drives
            // it, so a reply only the second source hears still keeps the QSO
            // moving.
            if (!key.isEmpty ())
              {
                if (m_actedOn.contains (key)) continue;
                m_actedOn.insert (key);
              }
            ...
            if (m_config.highlight_DXcall() && (m_hisCall != "") &&
              ...
              highlightInBothPanes (m_hisCall, QColor (255, 0, 0));  // highlight dxCallEntry
              QTimer::singleShot(500, [=] {                       // repeated highlighting to override JTAlert
                  highlightInBothPanes (m_hisCall, QColor (255, 0, 0));
              });
            ...
            if (m_config.highlight_DXgrid() && (m_hisGrid != "") && (decodedtext.string().contains(m_hisGrid))) {
              highlightInBothPanes (m_hisGrid, QColor (0, 0, 255));  // highlight dxGridEntry
            }
...
// The second source has its own Band Activity pane, and a double click there
// has to read that pane, not the first one.
void MainWindow::doubleClickOnCallIn2 (Qt::KeyboardModifiers modifiers)
{
  set_dateTimeQSO (-1);
  m_decodedText2 = true;
  m_clickedInput2Pane = true;
  doubleClickOnCall (modifiers);
  m_clickedInput2Pane = false;
  m_decodedText2 = false;
}
...
void MainWindow::doubleClickOnCall(Qt::KeyboardModifiers modifiers)
{
  ...
  // Each source has its own pane, so read the one that was actually clicked.
  // Reading the first pane for a click in the second returns whatever its
  // cursor happened to sit on, which is how a double click there started a
  // transmission without picking up the callsign.
  if(m_clickedInput2Pane) {
    cursor=ui->decodedTextBrowser_in2->textCursor();
  } else if(m_decodedText2) {
...
//        });
  ...
  // Point the second source at the station just clicked. Sync steers that
  // source's timing onto one station, and the station you have chosen to work
  // is the one worth hearing better, so the callsign goes in without waiting to
  // be typed. It only takes effect while Sync is ticked, and clearDX empties it
  // again so the second source is not left skewed towards a finished QSO.
  if (secondSourceActive ())
    {
      ui->leSyncCall->setText (ui->dxCallEntry->text ());
    }
}
...
void MainWindow::clearDX ()
{
  ...
  ui->dxGridEntry->clear ();
  // The sync target goes with the QSO: leaving it set would keep the second
  // source steered onto a station that is no longer being worked.
  ui->leSyncCall->clear ();
                            ...
                            , QString const& propmode, QByteArray const& ADIF)
{
  ...
  // Whatever was set for this one station goes with it. A report held by hand
  // would otherwise be sent unchanged to whoever called next, and a Sync steered
  // onto that station would leave the second receiver aligned to a timing that
  // is now nobody's. Sync on an empty callsign is a different thing - it holds
  // the second source against the first, which is meant to stay on - so it is
  // only switched off when it was actually steering onto someone.
  auto const was_steering = ui->cbSyncToSelected->isChecked ()
      && !ui->leSyncCall->text ().trimmed ().isEmpty ();
  clearDX();
  ui->cbHoldRpt->setChecked (false);
  if (was_steering) ui->cbSyncToSelected->setChecked (false);
  // Sync puts back the offset it started from, which for a QSO that began with
  // the second source already shifted is still a shift aimed at one station.
  // Zero is the only setting that is right for everybody, so that is where each
  // QSO leaves it.
  ui->sbInput2Offset->setValue (0.);
...
void MainWindow::on_actionFT4_triggered()
{
  ...
  if (!m_tci_audio) {
    ...
    setDetectorPeriods (m_TRperiod);  // TODO - not thread safe
  }
  ui->rh_decodes_title_label->setText(tr ("Rx Frequency"));
  updateBandActivityTitleLabel();
  ui->lh_decodes_headings_label->setText( "  UTC   dB   DT Freq    " + tr ("Message"));
...
void MainWindow::on_sbTR_valueChanged(int value)
{
  ...
  if(m_bFastMode or m_mode=="FreqCal" or m_mode=="FST4" or m_mode=="FST4W" or m_mode=="Q65") {
    ...
    if (!m_tci_audio) {
      ...
      setDetectorPeriods (value);  // TODO - not thread safe
    }
...
void MainWindow::on_actionClear_triggered() {
    if (ui->decodedTextBrowser->hasFocus()) {
        ui->decodedTextBrowser->erase();
    } else if (ui->decodedTextBrowser_in2->hasFocus()) {
        ui->decodedTextBrowser_in2->erase();
    } else {
        ui->decodedTextBrowser2->erase();
    }
...
void MainWindow::ZProcess ()
{
    ...
    if (!ui->autoButton->isChecked() && (ui->cbAutoCall->isChecked() || ui->cb_autoCallNext->isChecked())
        ...
        if (m_config.highlightDX() && !highlightCall.isEmpty()) {
            highlightInBothPanes (highlightCall, QColor (255, 0, 0));
        }
...
void MainWindow::pskReporterReportsUpdated(QStringList const& receiver_report_records) {
    ...
    // Both panes: a new decode already gets the underline in whichever pane it
    // lands in, but this refresh - which runs when the set of stations hearing
    // us changes - used to reach the first pane only, so the In 2 column went
    // on underlining stations that had stopped hearing us.
    for (auto const& oldCall : m_pskReporterReceivers) {
        if (!next.contains(oldCall)) {
            ...
            ui->decodedTextBrowser_in2->highlight_callsign_line(oldCall, QColor {}, QColor {}, false, false);
        }
    }
    for (auto const& call : next) {
        ui->decodedTextBrowser->highlight_callsign_line(call, QColor {}, QColor {}, false, true);
        ui->decodedTextBrowser_in2->highlight_callsign_line(call, QColor {}, QColor {}, false, true);
    }
