  ...
  m_lastPaintedX {-1}
{
  ...
  // No pop up menu on the right button any more: that button sets the transmit
  // frequency (see mouseReleaseEvent), and a menu would open on top of it. The
  // one item it held, "Set Rx & Tx Offset", is Ctrl-click.
...
// One button, one frequency: left moves receive, right moves transmit.
//
// WSJT-Z had made a plain left click move BOTH, with Ctrl for receive alone, so
// there was no way to nudge one without the other jumping with it - and the
// button that has always meant "transmit" only opened a menu holding a single
// item that moved both again. Ctrl-click still moves both together for anyone
// who wants that in one go, and Shift-click still sets transmit, as upstream.
void CPlotter::mouseReleaseEvent (QMouseEvent * event)
{
  auto const button = event->button ();
  if (Qt::LeftButton != button && Qt::RightButton != button)
    {
  ...
  // FST4W has no receive frequency to place, so every click there sets transmit.
  if (Qt::RightButton == button || shift || m_mode == "FST4W")
    {
